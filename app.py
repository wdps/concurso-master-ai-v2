            ...
            if resultados:
                ...
                for row in resultados:
                    ...
                    questao = {
                        ...
                        'alternativas': alternativas_dict, # Usar o dict deserializado
...
def criar_tabelas_se_necessario():
    ...
    try:
        ...
        # --- Otimização: Adicionar índices se não existirem ---
...
# CORREÇÃO: Nomes das funções (def) devem bater com o url_for()
...
def iniciar_simulado_api():
    """Endpoint para iniciar um novo simulado. (CORRIGIDO)"""
    try:
        ...
        config = {
            ...
            'quantidade_total': int(data.get('quantidade_total', 50)),
            'tempo_minutos': int(data.get('tempo_minutos', 180)),
...
def corrigir_redacao_api():
    ...
    try:
        ...
        nota_base = min(len(texto_redacao) / 10, 600)
        nota_final = min(int(nota_base + random.randint(200, 400)), 1000)
...
def serve_static(path):
    """Serve arquivos estáticos como (CSS, JS) ou o index.html da raiz."""
    if os.path.exists(os.path.join('static', path)):
        return send_from_directory('static', path)
    if os.path.exists(os.path.join('.', path)):
        return send_from_directory('.', path)
    
    # Se nada for encontrado, não retorne erro, mas sim o 'index.html' principal
    # Isso é comum em SPAs, mas aqui apenas evitamos o erro 404
    return render_template('index.html')
