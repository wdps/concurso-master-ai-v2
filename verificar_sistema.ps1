...
if (-not (Test-Path "questoes.csv")) {
    ...
    $exemploCSV = @"
id,disciplina,materia,enunciado,alternativas,resposta_correta,justificativa,dificuldade,peso,dica,formula
...
4,Matematica,Geometria,"Qual a formula da area do triangulo?","{""A"": ""base x altura"", ""B"": ""(base x altura)/2"", ""C"": ""lado x lado"", ""D"": ""pi x raio^2""}","B","Area do triangulo = (base x altura)/2","Media",2,"Lembre-se de dividir por 2","(b * h)/2"
5,Ciencias,Biologia,"O que e a fotossintese?","{""A"": ""Processo de digestao"", ""B"": ""Producao de energia com luz solar"", ""C"": ""Respiração celular"", ""D"": ""Reproducao das plantas""}","B","Fotossintese usa luz solar para produzir energia","Media",2,"Envolve luz solar",""
"@
    
    Set-Content -Path "questoes.csv" -Value $exemploCSV -Encoding UTF8
    Write-Success "Arquivo questoes.csv de exemplo criado com 5 questões"
} else {
...
try:
    ...
    for i, q in enumerate(questions, 1):
        ...
        # Verificar JSON das alternativas
        try:
            ...
            try:
                ...
                if cursor.fetchone() is None:
                    cursor.execute('''
                        ...
                        (disciplina, materia, enunciado, alternativas, resposta_correta, 
                    ...
                    ''', (
                        ...
                        row.get('materia', ''),
                        row.get('enunciado', ''),
                        row.get('alternativas', '{}'),
                        row.get('resposta_correta', ''),
                        row.get('justificativa', ''),
                        row.get('dificuldade', 'Media'),
                        float(row.get('peso', 1)),
                        row.get('dica', ''),
                        row.get('formula', '')
                    ))
                    imported += 1
...
"@
    ...
    $replacements = @{
        ...
        'logger\.info\("✅' = 'logger.info("SUCESSO'
        'logger\.error\("❌' = 'logger.error("ERRO'
        'logger\.warning\("⚠️' = 'logger.warning("AVISO'
    }
...
<body>
    ...
    <div id="content">
        <p>Conteudo da pagina $template</p>
    </div>
</body>
</html>
"@
...
Write-Host "`n🚀 INICIANDO ESQUEMATIZA.AI..." -ForegroundColor Green
Write-Host "📍 Acesse: http://127.0.0.1:5001" -ForegroundColor White
